...
from utils.dataset import MelDataset, mel_spectrogram, get_dataset_filelist
from utils.stft_loss import MultiResolutionSTFTLoss
from model.wavnet import Generator, generator_loss
from model.discriminator import SpecDiscriminator, MultiScaleDiscriminator, feature_loss,   discriminator_loss
from utils.utils import plot_spectrogram, scan_checkpoint, load_checkpoint, save_checkpoint
from utils.yaml_function import HParam
...
def train(rank, args, hp, hp_str):
    ...
    for epoch in range(max(0, last_epoch), args.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            x = x.unsqueeze(1)
            y = y.unsqueeze(1)
            before_y_g_hat, y_g_hat = generator(x)
...
def main():
    ...
    parser.add_argument('--train_file', default='data/4kHz/train_set.txt')
    parser.add_argument('--valid_file', default='data/4kHz/train_set.txt')
    parser.add_argument('--checkpoint_path', default='cp_hifigan')
    parser.add_argument('-c', '--config', default='utils/config.yaml')
